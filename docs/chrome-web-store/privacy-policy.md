...
DeepSeek++ is a browser extension that enhances DeepSeek chat workflows with user-controlled memory, skills, prompt presets, MCP tools, inline tool execution, conversation export, automation, and optional side-panel chat.
...
DeepSeek++ has one purpose: to enhance DeepSeek chat workflows with memory, skills, presets, tools, conversation export, automation, and optional side-panel chat that the user controls.
...
Most extension data is stored locally in the user's browser using extension storage and IndexedDB. This includes memories, custom skills, presets, settings, automation tasks, MCP configuration, DeepSeek API Key, and tool execution history.
...
DeepSeek++ 是一个浏览器扩展，用于增强 DeepSeek 对话工作流，提供用户可控的长期记忆、技能、提示词预设、MCP 工具、内联工具执行、对话导出、自动化任务和可选侧边栏对话。

本隐私政策说明扩展会处理哪些数据、如何使用这些数据、何时可能传输数据，以及用户可以如何控制自己的数据。

## 1. 单一用途

DeepSeek++ 的单一用途是增强 DeepSeek 对话工作流，提供由用户控制的记忆、技能、预设、工具、对话导出、自动化和可选侧边栏对话能力。
...
大多数扩展数据默认通过浏览器扩展存储和 IndexedDB 保存在用户浏览器本地，包括记忆、自定义技能、预设、设置、自动化任务、MCP 配置、DeepSeek API Key 和工具执行历史。
